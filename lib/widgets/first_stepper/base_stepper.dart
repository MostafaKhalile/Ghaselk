...
import 'package:ghaselk/widgets/first_stepper/dotted_line.dart';
...
class _BaseStepperState extends State<BaseStepper> {
  ...
  void _listener() {
    var counter = context.read<Counter>();
    if (_selectedIndex < counter.value) {
      next();
    } else {
      previous();
    }
  ...
  Widget _customizedIndicator(int index) {
    return Consumer<Counter>(
        builder: (context, counter, child) => BaseIndicator(
                  ...
                  ? () {
                      if (widget.steppingEnabled) {
                        setState(() {
                          ...
                          if (widget.onStepReached != null) {
                            widget.onStepReached(counter.value);
                          }
                        });
                      }
                    }
              ...
              borderColor: widget.stepBorderColor,
  ...
  Widget _nextButton() {
    return IgnorePointer(
        ...
        child: IconButton(
            ...
            onPressed: () {
              var counter = context.read<Counter>();
              if (counter.value < widget.children.length - 1 &&
